...
run_pdf() {
    ...
    local compression_quality=$(get_config "pdf" "quality" "85")
    ...
    [[ "$compression_quality" =~ ^[0-9]+$ ]] || compression_quality=85
    ...
    if [[ ${#inputs[@]} -eq 0 ]]; then
        ...
        echo "   -q <quality>     : JPEG Quality for compressed version (default 85)."

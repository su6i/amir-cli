...
async function renderMarkdown(inputPath, outputPath, fontPath, userDataDir, freeSize, pageWidthArg, pageHeightArg, themeArg, forceRtlArg) {
    // --force-rtl : force whole document RTL, disable per-element LTR flipping
    const forceRtl = String(forceRtlArg).trim() === 'true' || String(forceRtlArg).trim() === 'force-rtl';
    ...
    const isLtrPrimary = forceRtl ? false : (enCount > faCount);
...
` + themeCSS;
    ...
    await page.evaluate((persianFont, forceRtl) => {
        // --force-rtl: keep every block RTL, never flip Latin-leading lines to LTR
        if (forceRtl) {
            document.querySelectorAll('p, li, ul, ol, h1, h2, h3, h4, h5, h6, th, td, blockquote').forEach(el => {
...
renderMarkdown(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8]).catch((err) => { console.error(err); process.exit(1); });

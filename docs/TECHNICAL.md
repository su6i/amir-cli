...
- **Engines:**
    ...
    - **xelatex:** Dedicated engine for `.tex` files. Run twice automatically to resolve TOC/references. Imports custom styles automatically from `lib/latex/`.
...
- **Key Technical Features:**
    ...
    - **Batch Build (`amir pdf build <folder>`):** Renders all `carousel.*.md` and `guide.*.md` files in a folder in one command. Applies the correct theme automatically, detects RTL languages (`fa`, `ar`) and adds `--force-rtl`, then merges outputs: carousels → `carousel.pdf`, guides → `guide.trilingue.pdf`. Language order for merging: `fr → en → fa` (then any extra langs alphabetically). Example: `amir pdf build posts/04_my_post`. Naming convention: `carousel.fr.md`, `carousel.en.md`, `carousel.fa.md` and `guide.fr.md`, `guide.en.md`, `guide.fa.md`.
    - **Fonts:** Both `carousel` and `guide` themes import **Inter** (Latin) and **Vazirmatn** (Persian/Arabic) from Google Fonts. Unicode fallback ensures Latin characters use Inter and Persian/Arabic characters use Vazirmatn automatically.

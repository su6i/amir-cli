...
| Command | Description |
| :--- | :--- |
| `amir download <URL> [opts]` | **Universal Downloader**: YouTube, Instagram (photos/carousels/reels), TikTok, Twitter/X, Vimeo and 1000+ sites. Instagram photo/carousel posts auto-detected and downloaded via gallery-dl (auto-installed); any video items inside a carousel are auto-normalized to H.264/AAC for macOS/QuickTime playback, same as the yt-dlp path. Key flags: `--resolution <h>`, `--formats`, `--subtitle`, `--yt-subs`, `--translate`, `--extreme`, `--get-link`, `--browser`, `--cookies`, `--normalize`, `--po-token`, `--yt-dlp-args`. Instagram images: `--format jpg\|png\|webp` (default: jpg). |

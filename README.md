...
| `amir init-project [dir]` | Scaffold a project: links `.agent/constitution` to one central clone (symlink, not a submodule — override its location with `AGENT_CONSTITUTION_DIR`, its clone URL with `AMIR_CONSTITUTION_URL`), standard dirs (`src/ tests/ docs/ assets/` with `.gitkeep`), curated `.gitignore` (enforces `.storage/`/`.env`/`.agent/constitution`), starter `CLAUDE.md`/`README.md`/`.env.example`, and — for Python/unspecified stacks — `uv init` (`pyproject.toml` + pinned interpreter). `TODO.md`/`SESSION.md` are created in the project's vault workspace (`~/.local/share/agent-projects/<slug>/workspace/`, never in the repo — rules 035/040/045); `TODO.md` opens with a First Session checklist. Installs the constitution `pre-commit`/`commit-msg` hooks (blocks `main` commits + enforces the docs checklist + no AI co-authorship) and refuses to nest inside an existing git repo (override: `AMIR_ALLOW_NESTED=1`). |

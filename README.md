...
| **`OPENAI_API_KEY`** | For `amir router models openai` (optional). | [OpenAI Platform](https://platform.openai.com/api-keys) |
...
codec:
  video: h264         # target video codec when normalizing downloads
  audio: aac          # target audio codec when normalizing downloads
...
| Command | Description |
| :--- | :--- |
| `amir download <URL> [opts]` | **Universal Downloader**: YouTube, Instagram (photos/carousels/reels), TikTok, Twitter/X, Vimeo and 1000+ sites. Output defaults to **H.264/AAC in MP4** on every path (owner ruling 2026-07-27) so it plays in QuickTime and elsewhere without extra codecs; anything else (HEVC, ProRes, VP9, AV1, Opus, ...) is transcoded automatically. Instagram photo/carousel posts auto-detected and downloaded via gallery-dl (auto-installed); video items inside a carousel are normalized the same way as the yt-dlp path. Also supports **private course sites you have purchased access to** (hostnames are configured locally and are not shipped in this repo — see `course_site` below): pass a single lesson URL or a whole course page and it downloads every lesson in document order as `NN - <title>.mp4`, using your own logged-in browser session. It never bypasses a login or paywall, and aborts outright if DRM is detected rather than attempting key handling. Key flags: `--resolution <h>`, `--formats`, `--subtitle`, `--yt-subs`, `--translate`, `--extreme`, `--get-link`, `--browser`, `--cookies`, `--refresh-cookies`, `--normalize`, `--keep-codec` (opt out of normalization, keep the source codec), `--po-token`, `--yt-dlp-args`, `--force` (course sites: re-download existing lessons). **Auth policy is anonymous-first**: cookies found implicitly (`./cookies.txt`, `cookies.file` in config, the default browser) are never sent on the first attempt and are only used to retry after it fails — a session-less cookie jar identifies your device to the site without logging it in, which earns a login wall and a rate-limit on that device id, strictly worse than sending nothing. Cookies you request explicitly (`--cookies <file>`, `--browser <name>`) are used on the first attempt; `--browser none` or `AMIR_NO_COOKIES=1` forces full anonymity. Cookies read from a browser are cached per browser+site under `~/.amir/cookies` (0600 in a 0700 dir) and reused until they go stale, so a repeat download does not decrypt the whole browser profile through the keychain again; only the site's own cookies are cached, never the full jar. Refresh on demand with `--refresh-cookies`, retune with `AMIR_COOKIE_CACHE_TTL` (seconds, default 43200) or `AMIR_COOKIE_CACHE_DIR`, and switch it off with `AMIR_NO_COOKIE_CACHE=1`. Codec policy is configurable via `~/.amir/config.yaml`'s `codec:` section; course-site hostnames, iframe video hosts and the lesson-link pattern are configurable via the `course_site:` section (or `AMIR_COURSE_SITE_DOMAINS` in `.env`). Instagram images: `--format jpg\|png\|webp` (default: jpg). Several yt-dlp copies can coexist (`.venv` pinned by `uv.lock`, `uv tool`, Homebrew), each upgraded by a different command; `amir` compares their versions on start-up and runs whichever is newest, so a stale copy can no longer shadow a fresh one. Site extractors rot within weeks — upgrade with `uv tool upgrade yt-dlp` (`brew upgrade yt-dlp` maintains a copy `amir` does not use), and once a copy is over 45 days old `amir` says so once a day. |
...
| `amir video convert <file> [--to fmt] [-o out] [--cpu]` | Convert video container format (MOV→MP4, MKV, WEBM, AVI). Defaults to stream-copy (instant, no quality loss). Handles Apple HEVC (`hvc1`) correctly — no tag mangling. Auto-detects mac-unplayable sources (vp9/av1 video, opus/vorbis audio) and forces a re-encode to h264/aac even without `--cpu`, so the output is always QuickTime-playable. `--cpu` re-encodes with libx264 CRF 23 — sharper text and slides, smaller output. |
...
#### 🎓 `amir research` — PhD/postdoc supervisor scout

A separate feature from `amir trend` (split out 2026-06-01, `lib/commands/research.sh`) — bridges to the same [Research Toolkit](https://github.com/su6i/research-toolkit)'s `professor_scout` pipeline, but for finding and reaching out to potential academic supervisors, not trending content.

| Command | Description |
| :--- | :--- |
| `amir research discover --keywords "MARL portfolio"` | Find potential supervisors by topic keywords (ArXiv + DBLP). Options: `--sources arxiv dblp`, `--since-year Y`, `--min-papers N`, `--top N`, `--format txt\|md\|xlsx`, `--categories`, `--profile FILE`, `--save FILE`. |
| `amir research professor --professor "Name" --institution "Lab"` | Deep-research one professor — papers → topic overlap → draft outreach email. Options: `--email`, `--gender M\|F`, `--lang fr\|en`, `--story TEXT`, `--since-year Y`, `--scholar-url URL`, `--profile FILE`, `--tracking FILE`, `--position-id ID`. |

Same `RESEARCH_TOOLKIT_DIR` setup as `amir trend` above.
...
| `amir scripts` | Interactive picker for saved one-off / low-traffic scripts — shows a numbered menu and prompts for a selection. |
| `amir scripts list` | List saved scripts without prompting. |
| `amir scripts <id> [args...]` | Run a saved script directly by its id, skipping the menu. Registry: `lib/config/scripts.txt` (`id\|description\|command` per line) — add new entries by editing that file, no code changes needed. `weather`, `qr`, `pass`, `dashboard`, `transfer`, `short`, `lock`, `unlock`, `speed` moved here 2026-07-13 (low real-world usage per `~/.zsh_history`) — still callable directly too (e.g. `amir pass`), just dropped from `amir help` and top-level tab-completion. |

...
| `amir pdf build <folder>` | **Batch Build**: Render all `carousel.*.md` and `guide.*.md` in a folder in one command. Applies `--theme carousel` / `--theme guide` automatically, detects RTL languages (`fa`, `ar`) and adds `--force-rtl`, merges carousels into `carousel.pdf` and guides into `guide.trilingue.pdf`. Language order: `fr → en → fa`. Example: `amir pdf build posts/04_my_post`. |

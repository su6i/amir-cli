...
| Command | Description |
| :--- | :--- |
| `amir download <URL> [opts]` | **Universal Downloader**: YouTube, Instagram (photos/carousels/reels), TikTok, Twitter/X, Vimeo and 1000+ sites. Output defaults to **H.264/AAC in MP4** on every path (owner ruling 2026-07-27) so it plays in QuickTime and elsewhere without extra codecs; anything else (HEVC, ProRes, VP9, AV1, Opus, ...) is transcoded automatically. Instagram photo/carousel posts auto-detected and downloaded via gallery-dl (auto-installed); video items inside a carousel are normalized the same way as the yt-dlp path. Key flags: `--resolution <h>`, `--formats`, `--subtitle`, `--yt-subs`, `--translate`, `--extreme`, `--get-link`, `--browser`, `--cookies`, `--normalize`, `--keep-codec` (opt out of normalization, keep the source codec), `--po-token`, `--yt-dlp-args`. Codec policy is configurable via `~/.amir/config.yaml`'s `codec:` section. Instagram images: `--format jpg\|png\|webp` (default: jpg). |

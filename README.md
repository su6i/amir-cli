...
| `amir init-project [dir]` | Scaffold a project: links `.agent/constitution` to one central clone (symlink, not a submodule — override its location with `AGENT_CONSTITUTION_DIR`, its clone URL with `AMIR_CONSTITUTION_URL`), standard dirs (`src/ tests/ docs/ assets/` with `.gitkeep`), curated `.gitignore` (enforces `.storage/`/`.env`/`.agent/constitution`), starter `CLAUDE.md`/`README.md`/`.env.example`, and — for Python/unspecified stacks — `uv init` (`pyproject.toml` + pinned interpreter). `SESSION.md` is created in the project's vault workspace (`~/.local/share/agent-projects/<slug>/workspace/`, never in the repo); tasks go ONLY to the central `_memory/TODO.md` under a `## <project>` section — no per-project TODO is created (rules 035/040/045/050). The generated `CLAUDE.md` points storage at the vault `data/` dir and contains no absolute personal paths. Installs the constitution `pre-commit`/`commit-msg` hooks (blocks `main` commits + enforces the docs checklist + no AI co-authorship) and refuses to nest inside an existing git repo (override: `AMIR_ALLOW_NESTED=1`). |
